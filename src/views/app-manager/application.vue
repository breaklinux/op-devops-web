  ...
  <div class="app-container">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogCicdFormVisible">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <!--<el-button type="success" @click="confirmCicd()">-->
...
import { getProjectList, getEnvList, getReleaseList, getDevelopmentLanguageList, postSoftpackage, postDeployCicdPushData, postApiList, deleteHost, putEditHost, getInstanceIP } from '@/api/application'
export default {
  data() {
    return {
      options:[],
  ...
  methods: {
    ...
    deployCicdPushData: function(formName) {
      this.cicd.giturl = this.temp.giturl
       this.$refs[formName].validate((valid) => {
          if (valid) {
               postDeployCicdPushData(this.cicd).then((response) => {
                console.log('发布发送请求成功')
                console.log(response)
                if (response.callbackUrl) {
                  this.$router.push({ path: '/cicd/pipline', query: {
                    a:1
                  }})
    ...
    get_developmentLanguageList() {
      getDevelopmentLanguageList().then((response) => {
        this.languageList = response.data
      })
    },
    get_softpackage() {
      this.cicd.appname = this.temp.appname
      postSoftpackage(this.cicd).then((response) => {
    ...
    getList() {
      ...
      getProjectList(obj).then(response=> {
    ...
    postList() {
      var _this50 = this
      postApiList(this.temp).then(function(response) {
    ...
    delHost(row) {
      ...
      _this51.$confirm('此操作将永久删除该条数据, 是否继续?', '提示', {
        ...
        deleteHost(row).then(function(response) {
    ...
    updateHost() {
      ...
      _this52.$confirm('此操作将修改该条数据, 是否继续?', '提示', {
        ...
        putEditHost(_this52.temp).then(function(response) {
          console.log(response)
    ...
    selectChange(val) {
      ...
      getInstanceIP(this.temp.appname,this.cicd.env).then(res => {
        var msg = res.msg
        let ipList = res.data;
         if(msg === 'success') {
